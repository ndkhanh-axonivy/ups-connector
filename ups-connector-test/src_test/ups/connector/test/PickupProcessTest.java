...
public class PickupProcessTest{
  ...
  void cancelPickup(BpmClient bpmClient) throws NoSuchFieldException {
    ...
    ExecutionResult result = bpmClient.start().subProcess(startable).execute("1", "ciewssoatcnc0lRzHj9P2z", "testing", "2929602E9CP", "02");
  ...
  @Test
  void pickupPendingStatus(BpmClient bpmClient) throws NoSuchFieldException {
    BpmElement startable = testee.elementName("pickupPendingStatus(String,String,String,String,String)");
    ExecutionResult result = bpmClient.start().subProcess(startable).execute("1", "ciewssoatcnc0lRzHj9P2z", "testing", "F22H44", "01");
    var response = (PickupPendingStatusResponse) result.data().last().get("PickupPendingStatusResponse");
    assertThat(response.getPendingStatus().get(0).getPickupType()).isEqualTo("01");
    assertThat(response.getResponse().getResponseStatus().getDescription()).isEqualTo("Success");
  }
}
